...
class HyperpayPlugin {
  ...
  static const MethodChannel _channel =
      const MethodChannel('plugins.mohamedElbaz/hyperpay');
  ...
  Future<String> get getCheckoutID async {
    try {
      final body = {
        'entityId': "8acda4c98262a03e018274272d9a5c65",
        // _checkoutSettings?.brand.entityID(config),
        'amount': 1.0,
        'currency': 'SAR',
        "STC_PAY": 1,
        "phone": _checkoutSettings?.phone,
      ...
      print("Bodyyy" + _resBody.toString());
  ...
  Future<PaymentStatus> pay(CardInfo card) async {
    try {
      ...
      log('GettttStaussss', name: "HyperpayPlugin/paymentStatus");
    ...
    try {
      ...
      final Response response = await post(
        Uri(
            ...
            queryParameters: {"id": _checkoutID}),

        // Uri(  _config.statusEndpoint.scheme,  _config.statusEndpoint.host),
        headers: headers,
        body: (_checkoutSettings?.headers['Content-Type'] ?? '') ==
                'application/json'
            ? json.encode(body)
      ...
      log('GettttStaussss bodyyy $_resBody',
          name: "HyperpayPlugin/paymentStatus");
      try {
